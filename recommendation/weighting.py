...
from goodBuy_web.models.blacklist import BlackList  # 假設這是黑名單模型
import random
...
def compute_shop_scores(user, shop_queryset=None):
    ...
    def _add_score(sid, score):
        if shop_ids is None or sid in shop_ids:
            scores[sid] += score
    ...
    for kw in recent_keywords:
        for s in Shop.objects.filter(name__icontains=kw):
            _add_score(s.id, 5)
    ...
    fav_shop_ids = Product.objects.filter(id__in=fav_pids).values_list('shop_id', flat=True)
    for sid in fav_shop_ids:
        _add_score(sid, 3)

    # 購買過商品的商店
    bought_pids = Order.objects.filter(user=user).values_list('product_id', flat=True)
    bought_shop_ids = Product.objects.filter(id__in=bought_pids).values_list('shop_id', flat=True)
    for sid in bought_shop_ids:
        _add_score(sid, 2)

    # 最近看過商店（超過冷卻期者才加分）
    recent_views = ShopFootprints.objects.filter(user=user, date__lt=cooldown)
    for sid in recent_views.values_list('shop_id', flat=True):
        _add_score(sid, 4)
...
def personalized_homepage_shops_final(user, limit=20):
    ...
    level1_ids = [sid for sid in new_ids if Shop.objects.filter(id=sid, permission__id=1).exclude(owner_id__in=excluded_owner_ids).exists()]
    ...
    hot_ids = [
        ...
        if s.id not in seen_shop_ids and s.owner_id not in excluded_owner_ids
    ]
    ...
    recent_related_ids = [sid for sid in recent_related_ids if sid not in level1_ids + level2_ids and Shop.objects.filter(id=sid, permission__id=1).exclude(owner_id__in=excluded_owner_ids).exists()]
